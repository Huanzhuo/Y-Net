...
def sdr(estimation, origin, masks):
    """
    ...
    """
    estimation = estimation.unsqueez(2) # (batch, chs, 1, nsample)
    ...
    if mask is not None:
        sdr = (sdr * masks).sum(dim=(0, -1)) / mask.sum(dim=(0, -1)).clamp(min=1e-8)  # shape: (4)
    else:
...
# def calculate_loss(estimation, origin, mix, mask=None):
#     sdr = sdr(estimation, origin, mask)
#     loss = -sdr.sum()
#     estimated_mix = estimation.sum(1)
#     reconstruction_sdr = sdr(estimated_mix, mix).mean()
#     loss += -reconstruction_sdr
#     return loss
...
class Criterion(nn.Module):
    ...
    def forward(self, target, est):
        ...
        # sdr_est_target_inter = calc_sdr_torch(est_inter, target.squeeze()).sum()
        # sdr_est_target_intra = calc_sdr_torch(est_intra, target.squeeze()).sum()
        loss_1 = self.loss(est_inter, target.squeeze())
        loss_2 = self.loss(est_intra, target.squeeze())
        loss = self.alpha_1 * loss_1 + self.alpha_2 * loss_2
