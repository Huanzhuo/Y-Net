...
class TCNBlock(nn.Module):
    def __init__(self, ch_in, ch_mid, ch_out, k, p, d, skip=True, ac_func='PRelu'):
...
class BottleneckTCN(nn.Module):
    def __init__(self, ch_in, ch_mid, ch_out, n=1, ac_func='PRelu', skip=True):
        ...
        for i in range(n):
            d = 2 ** i
            p = d
            tcn_blocks += [TCNBlock(ch_in, ch_mid, ch_out, k=3, p=p, d=d, skip=skip, ac_func=ac_func)]
